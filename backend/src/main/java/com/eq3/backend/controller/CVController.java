...
public class CVController {
    ...
    @PostMapping(value = "/save/CV/{username}",
            produces = "application/json;charset=utf8",
            consumes = { "multipart/form-data" })
    public ResponseEntity<Student> saveCV(@RequestParam("document") MultipartFile multipartFile,
                                          @PathVariable(name = "username") String username) {
        return service.saveCV(username, multipartFile)
    ...
    @GetMapping("/getAll/student/CVS/{username}")
    public ResponseEntity<List<CV>> getAllStudentsWithActiveAndNotValidCV(@PathVariable String username) {
        return service.getAllCVS(username)
